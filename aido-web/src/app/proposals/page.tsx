  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        ...
        <div className="flex flex-wrap gap-3">
          {isConnected && (
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-[#E2DFD9] bg-white px-5 py-2.5 text-sm font-semibold text-[#1A1613] transition-all hover:bg-[#F0EEEB]"
            >
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1.2fr_1fr]">
        <div className="rounded-2xl border border-[#E2DFD9] bg-white p-6 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-[#1A1613]">DAO Overview</h2>
          </div>
        ...
        <div className="rounded-2xl border border-[#E2DFD9] bg-white p-6 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-[#1A1613]">Governance Settings</h2>
          </div>
      ...
      <div className="mt-8 space-y-4">
        {loading ? (
          <div className="rounded-2xl border border-[#E2DFD9] bg-white p-10 text-center shadow-sm">
            <p className="text-[#8C8680]">Loading proposals from Monad testnet...</p>
          </div>
        ) : error ? (
          ...
          <div className="rounded-2xl border border-[#E2DFD9] bg-white p-8 shadow-sm">
            <h3 className="text-lg font-bold text-[#1A1613]">No on-chain proposals yet</h3>
            <p className="mt-2 text-sm leading-relaxed text-[#8C8680]">
              The DAO is live, but this governor has not emitted any <code className="rounded bg-[#F0EEEB] px-1.5 py-0.5 text-[11px] text-[#1A1613]">ProposalCreated</code> events yet.
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              <Link
                href="/profile"
                className="rounded-xl border border-[#E2DFD9] bg-white px-5 py-2.5 text-sm font-semibold text-[#1A1613] transition-all hover:bg-[#F0EEEB]"
              >
                Setup Profile
              </Link>
            </div>
            ...
            <div className="mt-6 rounded-xl bg-[#F8F7F4] p-4">
              <p className="text-xs font-semibold uppercase tracking-wider text-[#8C8680]">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {[
                  ...
                  <div
                    key={label}
                    className="rounded-lg border border-[#E2DFD9] bg-white px-3 py-2 text-xs"
                  >
                    <p className="font-semibold text-[#1A1613]">{label}</p>
                    <p className="mt-1 font-mono text-[#8C8680]">{shortAddress(value)}</p>
                  </div>
                ))}
              </div>
            </div>
...
function ProposalCard({ proposal }: { proposal: Proposal }) {
  ...
  return (
    <Link
      ...
      className="block rounded-2xl border border-[#E2DFD9] bg-white p-6 shadow-sm transition-all hover:border-[#6C5CE7]/30 hover:shadow-md"
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <p className="font-bold text-[#1A1613]">{proposal.description}</p>
          <div className="mt-2 flex flex-wrap gap-3 text-xs text-[#8C8680]">
        ...
        <span
          className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-semibold ${
            stateColor[stateLabel] ?? "bg-[#F0EEEB] text-[#8C8680]"
          }`}
        >
          {stateLabel}
        </span>
      ...
      {votes && (
        <div className="mt-4 flex flex-wrap gap-4 text-xs font-medium text-[#8C8680]">
...
function InfoRow({
  ...
  return (
    <div className="rounded-xl bg-[#F8F7F4] px-4 py-3">
      <p className="text-xs font-semibold uppercase tracking-wider text-[#8C8680]">{label}</p>
      <p className={`mt-1 text-sm text-[#1A1613] ${mono ? "font-mono" : ""}`}>{value}</p>
    </div>
  );
}
